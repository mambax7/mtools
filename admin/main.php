...
function mtools_setup()
{
    ...
    foreach ($xoopsConfig['theme_set_allowed'] as $theme) {
        ...
        if (is_file(XOOPS_ROOT_PATH . "/themes/{$theme}/config.php")) {
            $theme_kind = '';
            require_once XOOPS_ROOT_PATH . "/themes/{$theme}/config.php";
            if (!empty($theme_kind)) {
                if (empty($tt_theme_kind_arr[$theme]) or 0 === $theme_change) {
                    $sql = 'update `' . $xoopsDB->prefix('mtools_setup') . "` set `tt_theme_kind`='{$theme_kind}' where `tt_theme`='{$theme}'";
                    //die($sql);
...
function search_bootstrap($path = '')
{
    ...
    if (false !== mb_strpos($file_str, 'bootstrap')) {
        return 'bootstrap';
    }
...
function mk_bootstrap_menu_options($theme_kind = '', $mode = 'light')
{
    ...
    $dir = XOOPS_ROOT_PATH . "/modules/mtools/{$theme_kind}/themes/{$mode}/";
...
//列出目錄檔案
/**
 * @param string $directory_base_path
 * @return array
 */
function directory_list($directory_base_path = '')
{
    ...
    $allfile = glob($directory_base_path . '*', GLOB_NOSORT);
...
function save()
{
    ...
    foreach ($tt_use_bootstrap as $tt_theme => $tt_use_bootstrap) {
        ...
        $sql = 'replace into `' . $xoopsDB->prefix('mtools_setup') . "` (`tt_theme` , `tt_use_bootstrap`,`tt_bootstrap_color` , `tt_theme_kind`) values('{$tt_theme}', '{$tt_use_bootstrap}', '{$tt_bootstrap_color}', '{$tt_theme_kind}')";
        $xoopsDB->queryF($sql) or Utility::web_error($sql, __FILE__, __LINE__);
    }
}
/*-----------執行動作判斷區----------*/
